...
def _process_url(
    url: str,
    sanitize: bool = True,
    model: str = "qwen3:4b",  # ← Standardized to qwen3:4b
    raw: bool = False,
    strict: bool = False,
) -> str:
    ...
    if sanitize:
        return chunk_and_sanitize(cleaned, model)
    return cleaned
...
def fetch(
    ...
    model: str = typer.Option("qwen3:4b", "--model", "-m", help="Ollama model"),  # ← Standardized to qwen3:4b
    ...
    try:
        final = _process_url(url, sanitize, model, raw, strict)
    except Exception as e:
...
def serve(
    ...
    model: str = typer.Option("qwen3:4b", "--model", "-m", help="Default Ollama model"),  # ← Standardized to qwen3:4b
):
...
def test():
    ...
    sanitized_output = chunk_and_sanitize(cleaned_sample, model="qwen3:4b")  # ← Standardized to qwen3:4b
